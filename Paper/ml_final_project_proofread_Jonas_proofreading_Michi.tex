...
\missingfigure[figwidth=0.5\textwidth]{Why is there no image of a cookie?! :-(}
...
The classification of a new sample using a single Fern is very efficient: The previously stored tests are applied to the tested sample in the same order to find a binary number representing the index of the bucket into which this sample drops. The combined buckets of all classes, after normalizing\comment{normalization}, then represent the corresponding class posterior distribution given the features of the tested sample. This means that every Fern knows with which certainty a sample would be of which class.

For a Ferest, the results of all Ferns are combined using the Naive Bayes' assumption - which is now more appropriate as all Ferns check on random features and thresholds. So instead of taking the best vote as a Forest does\comment{as done by Forests...klingt das toller!?}, a Ferest multiplies all Fern probabilities to find the class that is most likely - as a result, Ferns that were not sure about their classification can be outvoted more easily as each Fern's certainty is incorporated.\\
...
Nevertheless, the depth of a Fern can also become critical when chosen\comment{when being chosen!?} too large. Where Trees tend to overfit for such settings, the number of buckets for the histograms of a Fern increases dramatically. Accordingly, very few samples drop into each bucket while training and therefore, the class-conditional probability distribution is close to being uniform. Informally, every Fern is then uncertain to which class a new sample might belong.\\
This shows that when increasing the depth of a Ferest\comment{the depth of the Ferns of a Ferest/the depth of a Ferest's Ferns...}, the number of samples in the training set should increase exponentially - for our chosen training set, the best depth seems to be ten.
...
Additionally to the original 'completely' random version of the Ferest, we implemented \textit{bestGini} and \textit{meanRandom}. But where training of a single Fern of the same depth took only 15 seconds for 'random' implementations, it already lasted 20 minutes for the heuristical option incorporating costs with Gini as impurity measure. Nevertheless, \textit{bestGini} was still faster\comment{than Forests!? das sollte mMn hier noch explizit stehen. ansonsten bezieht mans noch auf die anderne varianten der ferests} for the training by orders of magnitude (see next section about Comparison).
