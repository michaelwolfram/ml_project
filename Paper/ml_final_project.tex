...
\section{Approach}
% 'what you did'
We already know about Trees and Forests from the lecture - additionally we read of a Machine Learning algorithm called \textit{Ferns} [1], which was recently (2007) introduced to detect objects in images. Ferns have a very similar behaviour as Forests and, in fact, are completely replaceable, i.e. intentionally building a Tree with the same tests in each layer makes it interchangeable with a Fern containing the same binary tests - though the important difference lies in the striking evaluation speed of Ferns for the classification of new samples. 
